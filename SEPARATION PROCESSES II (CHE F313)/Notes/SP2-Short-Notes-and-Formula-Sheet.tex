...
\midrule
Anthracite coal  & \ensuremath{27^{\circ}}          && Granular solids & \ensuremath{15^{\circ}}--\ensuremath{30^{\circ}}\\
Fine sand        & \ensuremath{31^{\circ}}          && Cohesive solids & $\simeq\ensuremath{90^{\circ}}$\\
Bituminous coal  & \ensuremath{35^{\circ}}          &&&\\
Wet clay         & \ensuremath{17^{\circ}}          &&&\\
Gravel           & \ensuremath{39^{\circ}}--\ensuremath{48^{\circ}}&&&\\
\bottomrule
\end{tabular}
\end{center}
The angle of repose is the pile-side angle with the horizontal; the angle of internal friction is the slope angle of the shear-strength $\tau$ vs normal-stress $\sigma_n$ line (cohesive solids sheared at the unconfined surface fail nearly vertically, hence $\alpha_i\rightarrow\ensuremath{90^{\circ}}$).
\end{formulabox}
...
\begin{itemize}
  \item Clarifying filters remove \emph{small amounts} of solids or liquid droplets from either liquids or gases.
  ...
  \item Clarification differs from screening in that the pores in the filter medium are larger --- sometimes much larger --- than the particles to be removed.
  \item \textbf{Liquid clarification}: clarifying filters for liquids include the gravity-bed filters used in water treatment; cake filters are also used extensively for clarification. In batch units the filtration rate and the filtration efficiency stay constant for a long period, and eventually reach a \textbf{breakthrough} value, after which backwashing or replacement becomes necessary.
  \item \textbf{Gas cleaning}: filters for gas cleaning include pad filters for atmospheric dust, and granular beds and bag filters for process dusts. Air is cleaned by passing it through pads of cellulose pulp, cotton, felt, glass fibre or metal screening; the pad material may be dry or coated with a viscous oil to act as a dust holder.
\end{itemize}

\spsub{Principles of clarification}
\begin{itemize}
  \item \textbf{Direct sieving}: the solid particles being removed completely plug the pores of the filter medium, and the rate of plugging is constant with time.
  \item \textbf{Standard blocking}: the particles partially block the pores, giving a gradual reduction in pore size.
...
\begin{itemize}
  \item The feed is passed through a membrane or bed, the solids being trapped in the filter and the filtrate released at the other end.
  \item The majority of the feed flow travels \emph{tangentially across the surface} of the filter, rather than into the filter.
  \item The principal advantage is that the filter cake is substantially washed away during the filtration process, increasing the length of time that a filter unit can stay operational --- compare the dead-end arrangement, where all the feed is forced through the medium and the cake builds up on it.
  \item Applied to concentrate suspensions of fine particles or colloidal particles.
\end{itemize}
...
\begin{tabularx}{\linewidth}{@{}lX@{}}
\toprule
\textbf{Technique} & \textbf{Pore size and what is retained}\\
\midrule
Microfiltration (MF) & \ensuremath{10\,\mathrm{\mu m}}--\ensuremath{0.1\,\mathrm{\mu m}} (100\,nm): suspended solids, bacteria/algae and viruses are held back; water and dissolved ions pass.\\
Ultrafiltration (UF) & \ensuremath{0.1\,\mathrm{\mu m}}--\ensuremath{0.01\,\mathrm{\mu m}} (10\,nm): as MF, with viruses rejected more completely; ions still pass.\\
Nanofiltration (NF) & \ensuremath{10} nm--\ensuremath{1} nm: multivalent ions, viruses, bacteria/algae and suspended solids are rejected; monovalent ions and water pass.\\
Hyperfiltration / reverse osmosis (RO) & $<\ensuremath{1} nm$: water passes and essentially everything else is rejected --- monovalent ions, multivalent ions, viruses, bacteria/algae and suspended solids.\\
\bottomrule
\end{tabularx}}
\end{formulabox}
...
\begin{itemize}
  \item The \textbf{volume flux} is the superficial permeate velocity normal to the surface:
\end{itemize}
\begin{formulabox}
\begin{align*}
...
\begin{itemize}
  \item As permeate is drawn through the membrane, solute is carried towards the surface faster than it can diffuse back, so the concentration at the surface rises above the bulk value. In \textbf{RO} the surface concentration is only moderately higher than the bulk concentration; in \textbf{UF} the surface concentration is large, and a gel layer forms.
  \item The boundary layer has thickness $\delta$. Writing $C_s$ for the concentration at the surface ($x=0$) and $C_1$ for the bulk concentration at $x=\delta$, with $C_2$ the solute concentration in the permeate:
\end{itemize}
\begin{formulabox}
\begin{align*}
J_v&=K_c\ln\frac{C_s-C_2}{C_1-C_2}
 &&\text{volume flux with concentration polarization}
\end{align*}
\vspace{2pt}
{\small $C_s$ = concentration at the surface ($x=0$);\quad $C_1$ = bulk concentration at $x=\delta$, the edge of the gel boundary layer;\quad $C_2$ = solute concentration in the permeate.}
\end{formulabox}
...
\end{align*}
\vspace{2pt}
{\small $K_c$ in \ensuremath{\mathrm{c m}/\mathrm{ s}}, $D_v$ (diffusivity) in \ensuremath{\mathrm{c m}^{2}/\mathrm{ s}}, $L$ a characteristic length; the boxed correlation is the one the deck uses to get $K_c$.}
\end{formulabox}
...
\begin{itemize}
  \item A modified disk-type centrifuge with a \emph{double conical} bowl. At the periphery of the bowl, at its maximum diameter, is a set of small holes or nozzles, perhaps \ensuremath{3\,m\,m} in diameter.
...
\begin{itemize}
  \item A particle of a given size is \emph{removed} when it reaches the wall of the separator bowl. The particle moves radially outward with its terminal velocity, and the diameter of the smallest particle that is removed is what the design calculates.
  ...
  \item Geometry: $r_1$ = radius of the inner surface of the liquid (the liquid interface), $r_2$ = inside radius of the bowl, $r_A$ = radius at which the feed enters, $r_B$ = radial position reached by the particle at the end of settling, $b$ = height of the bowl.
  \item If $r_B<r_2$ the particle leaves the bowl with the liquid, i.e.\ it is \emph{not} removed. If $r_B=r_2$ the particle has been deposited on the bowl wall and is removed from the liquid.
\end{itemize}

\spsub{Residence time and settling time --- Stokes' law regime}
\begin{formulabox}
\begin{align*}
\text{Terminal velocity at radius }r:\quad & u_t=\frac{\omega^{2}(\rho_p-\rho)D_p^{2}}{18\mu}
...
\end{align*}
\textbf{Cut point} $D_{pc}$: the diameter of the particle that reaches \emph{half} the distance between $r_1$ and $r_2$ during the settling time, i.e.
\[r_B=r_2,\qquad r_A=\frac{r_1+r_2}{2}\]
so that
\[\boxed{\;q_c=\frac{\pi b\,\omega^{2}(\rho_p-\rho)D_{pc}^{2}}{18\mu}\,
   \frac{r_2^{2}-r_1^{2}}{\ln\left[\dfrac{2r_2}{r_1+r_2}\right]}\;}
\]
Particles larger than $D_{pc}$ are removed at this flow rate; particles smaller than $D_{pc}$ stay in the liquid.
\end{formulabox}
...
Writing $s$ for the settling distance and $t_T$ for the settling time, $u_t=\dfrac{s}{t_T}$ and $t_T=\dfrac{V}{q_c}$, so
\[q_c=\frac{2V\,u_t}{s}
 =\frac{2V}{\omega^{2}}\cdot\frac{\omega^{2}D_p^{2}(\rho_p-\rho)r_2}{18\mu\,s}
 =\boxed{\;\frac{2V\omega^{2}r_2D_p^{2}(\rho_p-\rho)}{18\mu\,s}\;}\]
\end{formulabox}
...
\end{formulabox}
\notetext{Because $q_c=2\Sigma u_g$, a centrifuge and a gravity settler that must do the same job are compared through $\Sigma$ alone: double $\Sigma$ and the same cut is achieved at double the flow rate. The deck's Table 30.5 lists $\Sigma$ values for commercial sedimenting centrifuges (tubular, disk, helical-conveyor, axial-flow-conveyor types) against bowl diameter and speed.}

\spsub{Worked problem set by the deck (Mod 2 Lec 8, slide 26)}
\begin{daybox}
\textbf{Capacity of a clarifying centrifuge.} What is the capacity in cubic metres per hour of a clarifying centrifuge operating under the following conditions? Bowl diameter \ensuremath{600\,m\,m}; thickness of the liquid layer \ensuremath{75\,m\,m}; depth of bowl \ensuremath{400\,m\,m}; speed \ensuremath{1200\,r\,/\,m\,i\,n}; specific gravity of liquid $1.2$; specific gravity of solid $1.6$; viscosity of liquid \ensuremath{2\,c\,P}; cut size of particles \ensuremath{30\,\mathrm{\mu m}}.
\end{daybox}
